...
| `/settings/staff` | Staff accounts: invite, change role, switch access off (needs `MANAGE_STAFF`) |
...
**Home dashboard (phase 3).** `dashboard_providers.dart` and `dashboard_widgets.dart` hold the pieces: a three-step settlement tracker driven by `todaySettlement.status`; a terminal health strip (online ≤10 min, idle ≤24 h, offline) from `lastHeartbeat`; an approval-rate warning when today's rate is 15 points under the 30-day norm with the most common decline reason; "vs last {weekday}" from the timeseries endpoint with "vs yesterday" as fallback; quick actions (send / print last receipt, payment link behind its flag, call support from config); daily and monthly sales targets (stored in merchant preferences as `dailyTarget`/`monthlyTarget`, mirrored locally) with a progress ring on the hero; and activity filter chips. Users lacking `VIEW_SETTLEMENTS` get a cashier view: takings instead of balance, no wallet/settlement screens (also enforced by the router), and no company preferences without `MANAGE_PREFERENCES`.
...
| GET / POST / PUT | `/api/v1/merchant/staff`, `/api/v1/merchant/staff/{id}` | Staff list, invite, role and access changes |
