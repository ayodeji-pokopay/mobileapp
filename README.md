...
| `/settings/staff` | Staff accounts: invite (with card-machine scope for cashiers), change role, switch access off, resend or remove an invite (needs `MANAGE_STAFF`) |
| `/settings/staff/activity` | Staff activity log (invites, role and access changes) |
| `/accept-invite?code=` | Public: an invited staff member sets name and password with their emailed code |
...
**Home dashboard (phase 3).** `dashboard_providers.dart` and `dashboard_widgets.dart` hold the pieces: a three-step settlement tracker driven by `todaySettlement.status`; a terminal health strip driven by the backend's `health` verdict (HEALTHY / DEGRADED / OFFLINE, with `healthReasons` mapped to friendly copy in `terminal_detail_sheet.dart`) and falling back to `lastHeartbeat` age on older backends; tapping a machine opens a detail sheet with battery, connection and signal, printer status, app version and last seen; an approval-rate warning when today's rate is 15 points under the 30-day norm with the most common decline reason; "vs last {weekday}" from the timeseries endpoint with "vs yesterday" as fallback; quick actions (send / print last receipt, payment link behind its flag, call support from config); daily and monthly sales targets (stored in merchant preferences as `dailyTarget`/`monthlyTarget`, mirrored locally) with a progress ring on the hero; and activity filter chips. Users lacking `VIEW_SETTLEMENTS` get a cashier view: takings instead of balance, no wallet/settlement screens (also enforced by the router), and no company preferences without `MANAGE_PREFERENCES`. `SHARE_RECEIPTS` gates send/print receipt, `EXPORT_REPORTS` the sales export, `MANAGE_TERMINALS` renaming card machines. A cashier with `terminalIds` on `/auth/me` sees only those machines: Home takings are summed from today's transactions on them, and recent activity, the terminal strip and My business are filtered.
...
| GET | `/api/v1/merchant/reports/transactions/by-decline-reason` | Top decline reason for the Home approval warning |
| GET / POST / PUT / DELETE | `/api/v1/merchant/staff`, `/api/v1/merchant/staff/{id}`, `…/{id}/resend-invite`, `…/activity` | Staff list, invite, role/access/terminal-scope changes, resend, remove, audit log |
| POST | `/api/v1/auth/accept-invite` | Invited staff finish sign-up (public) |
