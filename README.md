...
**Home dashboard (phase 3).** `dashboard_providers.dart` and `dashboard_widgets.dart` hold the pieces: a three-step settlement tracker driven by `todaySettlement.status`; a terminal health strip (online ≤10 min, idle ≤24 h, offline) from `lastHeartbeat`; an approval-rate warning when today's rate is 15 points under the 30-day norm with the most common decline reason; "vs last {weekday}" from the timeseries endpoint with "vs yesterday" as fallback; quick actions (send / print last receipt, payment link behind its flag, call support from config); daily and monthly sales targets (stored in merchant preferences as `dailyTarget`/`monthlyTarget`, mirrored locally) with a progress ring on the hero; and activity filter chips. Users lacking `VIEW_SETTLEMENTS` get a cashier view: takings instead of balance, no wallet/settlement screens (also enforced by the router), and no company preferences without `MANAGE_PREFERENCES`. `SHARE_RECEIPTS` gates send/print receipt, `EXPORT_REPORTS` the sales export, `MANAGE_TERMINALS` renaming card machines. A cashier with `terminalIds` on `/auth/me` sees only those machines: Home takings are summed from today's transactions on them, and recent activity, the terminal strip and My business are filtered.
